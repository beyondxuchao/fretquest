...
const GROUPS: Array<{id: Menu; title: string; options: readonly TrainingOption[]}> = [
  {id:'basic',title:'基础定位',options:[['locate','音名定位','看到音名，寻找指板位置'],['identify','反向识别','看到位置，选择正确音名'],['stringLocate','指定弦定位','只在指定弦上寻找目标音'],['allNotes','找出所有同名音','根据目标音名，找齐指板上的全部位置']]},
  {id:'relation',title:'关系训练',options:[['octave','八度形状','寻找上方一个八度位置'],['interval','音程定位','从根音寻找指定音程'],['intervalShape','音程形状','固定在相邻高音弦上找目标'],['earLocate','听音找指板','聆听真实吉他音，寻找相同音高']]},
...
const NAMES: Record<TrainingType,string> = {locate:'音名定位',identify:'反向识别',stringLocate:'指定弦定位',positionAssessment:'把位覆盖',allNotes:'找出所有同名音',octave:'八度形状',interval:'音程定位',intervalShape:'音程形状',earLocate:'听音找指板',adaptive:'自适应复习',scaleDegree:'音阶音级',chordTone:'和弦内音',arpeggio:'琶音路径'}
const DESCRIPTIONS: Record<TrainingType,string> = {locate:'看到音名，在整块指板上寻找位置',identify:'看到高亮位置，从十二个音名中选择',stringLocate:'只在指定的一根弦上寻找目标音',positionAssessment:'在指定的前、中、高把位范围内寻找音名',allNotes:'找齐当前范围内所有相同音名',octave:'从根音寻找上方一个八度的位置',interval:'从根音寻找指定的音程距离',intervalShape:'从根音到指定相邻高音弦，复现音程手形',earLocate:'聆听单音，在指板上找到相同实际音高',adaptive:'优先复习错误多、反应慢和长期未练的位置',scaleDegree:'把音阶级数映射到指定琴弦',chordTone:'识别并定位当前和弦的组成音',arpeggio:'按顺序连接和弦音，建立可演奏路径'}
