...
export function IntervalLearningPanel({intervals,semitones,anchorString,pair,noteAt,onSemitonesChange,onAnchorStringChange,onPlay}:Props){
  ...
  return <section className="interval-learning-panel">
    ...
    <div className="interval-lesson-controls"><div><span>选择音程</span><div className="interval-choice-grid">{intervals.filter((item)=>[3,4,5,7,10,12].includes(item[2])).map(([name,symbol,value])=><button key={symbol} className={semitones===value?'active':''} onClick={()=>onSemitonesChange(value)}><b>{symbol}</b><small>{name}</small></button>)}</div></div><div><span>相邻弦组合</span><div className="string-pair-choice">{[5,4,3,2,1].map((string)=><button key={string} className={anchorString===string?'active':''} onClick={()=>onAnchorStringChange(string)}>{string+1} 弦 → {string} 弦</button>)}</div></div></div>
    ...
    {pair&&<div className="interval-memory-guide"><span>形状规律</span><h3>{currentInterval?.[1]}：跨到相邻高音弦，再{fretShift===0?'保持同一品':`向${fretShift>0?'右':'左'}移动 ${Math.abs(fretShift)} 品`}，就是目标音。</h3><p>口诀：普通跨弦减 5，三弦到二弦减 4。{anchorString===2?'当前经过 3→2 弦，需要比普通形状向右补 1 品。':''}</p></div>}
  </section>
}
