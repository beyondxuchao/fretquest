...
function App() {
  ...
  return (
    <main className={`${focusTraining ? 'training-focus' : ''} ${scaleFocus ? 'scale-focus' : ''}`}>
      ...
      {appMode === 'training' && <TrainingNavigation trainingType={trainingType} status={status} onSelect={(type)=>{setTrainingType(type);setStatus('idle')}}/>}
