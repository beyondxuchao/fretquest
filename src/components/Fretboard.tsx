    ...
    <div className={boardClasses}>
      {activeStrings.map((active,string) => {
        ...
        return <div className={`string-row ${!enabled?'disabled':''} ${stringClass} ${highlightString===string||highlightStrings?.has(string)?'lesson-highlight-string':''}`} key={string} style={{gridTemplateColumns}}>
          <button className={`string-label ${openHit?(feedback.correct?'correct':'wrong'):''} ${openScaleClass} ${openLiveClass} ${openPlaybackClass} ${openStartClass} ${lessonHighlights?.[`${string}-0`]?`lesson-${lessonHighlights[`${string}-0`]}`:''} ${lessonActivePosition?.string===string&&lessonActivePosition.fret===0?'lesson-active-position':''}`} disabled={!enabled||Boolean(activeFretRange)} onClick={() => learningScale ? (openNote === scaleRootNote ? onSelectScaleStart({string,fret:0}) : onPlay(string,0)) : onChoose(string,0)} title={`${stringNames[string]} 空弦`}><span className="string-number-label">{string+1}弦</span><span className="open-note-label">{openDisplayNote}</span><b className={lessonHighlights?.[`${string}-0`]?'lesson-note-highlight':lessonActivePosition?.string===string&&lessonActivePosition.fret===0?'lesson-active-dot':lessonNaturalPositions?.has(`${string}-0`)?'lesson-natural-note':''}>{filteredExplore ? (openLessonVisible ? openDisplayNote : '') : appMode === 'learning'||status==='finished'||openScaleClass||openLiveClass?openDisplayNote:''}</b></button>
